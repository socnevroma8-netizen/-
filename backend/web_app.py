...
def home_page(request: Request, db=Depends(get_db)):
    ...
    lessons = [dict(row) for row in lessons_for_class(db, class_key) if row["day"] == today_name]
...
def section_page(section: str, request: Request, db=Depends(get_db)):
    ...
    sections = {
        ...
        "teachers": ("Преподаватели", "Здесь будет список преподавателей."),
